...
async function initializeApp() {
    ...
    // Set pier from URL or prompt
    if (!currentPier) currentPier = prompt('Enter pier name (e.g. Yamu, RPM):') || 'Default';

    const phuketDate = new Date().toLocaleDateString('en-GB', { timeZone: 'Asia/Bangkok' });
    document.getElementById('greeting').innerText = `${currentPier} POS`;
    document.getElementById('date-display').innerText = phuketDate;
...
async function apiRequest(endpoint, options = {}) {
    ...
    // Auth: API key takes priority, then Telegram initData, then URL token
    if (apiKey) {
    ...
    if (options.method === 'POST') {
        ...
        if (initData) urlObj.searchParams.append('initData', initData);
        if (authToken) urlObj.searchParams.append('token', authToken);
        if (options.data) {
...
async function refreshSessionStatus() {
    ...
    try {
        const result = await apiRequest('/api/v1/sessions/active', { data: { pier: currentPier } });
...
window.processSale = async function(type) {
    ...
    try {
        const result = await apiRequest('/api/v1/sales', {
            method: 'POST',
            body: {
...
async function loadReport() {
    if (!currentPier) return;
    try {
        const result = await apiRequest('/api/v1/sales/daily-report', { data: { pier: currentPier } });
